...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentsOfTextView(R.id.ResultText, "No game has been started");
    ...
    public void cm1(View view){
        String playerX = getInputOfTextField(R.id.PlayerXEnter);
        String playerO = getInputOfTextField(R.id.PlayerOEnter);
        String option = getItemSelected(R.id.spinner);
        String concatenate = "Current Game Board: " + "\n";
        game =  new Game (playerX,playerO);
        if (option.equals("Player O")){
        ...
        for (int i = 0; i < 3; i++) {
            for (int j= 0; j< 3; j++){
                concatenate += board[i][j] + " ";
            }
            concatenate += "\n";
        }
        concatenate += "Current Game Status: " + "\n" + game.getStatus();
        setContentsOfTextView(R.id.ResultText, concatenate);
    }

    public void cm2(View view){
        String row = getInputOfTextField(R.id.RowNumEnter);
        String col = getInputOfTextField(R.id.ColNumEnter);
        int rowNum = Integer.parseInt(row);
        int colNum = Integer.parseInt(col);
        String concatenate = "Current Game Board: " + "\n";
        game.move(rowNum,colNum);
        char[][] board = game.getBoard();
